...
#这个工具只接收一个输入：code。
    ...
    def run(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        normalized = self._normalize(code)#先把输入整理成一个比较标准的 Mermaid 形式
        valid, errors = self._validate_structure(normalized)#再检查结构是否合理
    ...
    def _normalize(self, code: str) -> str:#先做几件基础清洗
        code = code.strip()
