...
export default function App() {
  ...
  return <div className="app-shell"><header className="app-header"><div className="brand"><span className="brand-mark">A</span><div><h1>AutoFlow</h1><p>INTELLIGENT DIAGRAM STUDIO</p></div></div><div className="header-status"><span className="status-light" />SYSTEM READY</div></header>
    <main className="workbench"><aside className="composer-panel"><div className="panel-eyebrow">01 / INPUT LAB</div><div className="mode-grid">{MODES.map((item) => <button key={item.key} className={`mode-card ${mode === item.key ? "selected" : ""}`} onClick={() => setMode(item.key)}><strong>{item.label}</strong><span>{item.caption}</span></button>)}</div><div className="input-head"><div><h2>{activeMode.label}</h2><p>{activeMode.caption}</p></div><span className="direction-badge">{direction}</span></div>
      {isChatMode ? <div className="chat-area"><div className="chat-history">{currentChat.length ? currentChat.map((message, index) => <article key={index} className={`message ${message.role}`}><span>{message.role === "user" ? "YOU" : "AI"}</span>{message.title && <h3>{message.title}</h3>}{message.code ? <pre>{message.content}</pre> : <p>{message.content}</p>}</article>) : <div className="empty-state">从一个研究主题或清晰需求开始，AutoFlow 将把它组织为可验证的流程结构。</div>}</div><textarea value={chatInput} onChange={(event) => setChatInput(event.target.value)} placeholder={PLACEHOLDERS[mode]} rows="5" /></div> : <textarea className="editor" value={mode === "code" ? mermaidCode : input} onChange={(event) => mode === "code" ? setMermaidCode(event.target.value) : setInput(event.target.value)} placeholder={PLACEHOLDERS[mode]} />}
